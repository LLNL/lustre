				...
				struct osd_object *obj, struct osd_thandle *oh)
{
	...
	rc = -udmu_zap_cursor_init(&zc, &osd->od_objset, oid, 0);
	if (rc)
		goto out;
	...
	while (zap_cursor_retrieve(zc, za) == 0) {
		...
		rc = -zap_lookup(osd->od_objset.os, oid, za->za_name,
		       ...
		       struct osd_object *obj, struct osd_thandle *oh)
{
	...
	rc = -udmu_zap_cursor_init(&zc, &osd->od_objset, obj->oo_xattr, 0);
	if (rc)
		return rc;
	...
	while (zap_cursor_retrieve(zc, za) == 0) {
		...
		rc = -zap_lookup(osd->od_objset.os, obj->oo_xattr, za->za_name,
				 sizeof(uint64_t), 1, &xid);
		if (rc) {
			CERROR("%s: lookup xattr %s failed: rc = %d\n",
			       osd->od_svname, za->za_name, rc);
		} else {
			rc = __osd_object_free(&osd->od_objset, xid, tx);
			if (rc)
	...
	rc = __osd_object_free(&osd->od_objset, obj->oo_xattr, tx);
	if (rc)
