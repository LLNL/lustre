...
/*
 * Fix up a set of pages, each page containing an array of lu_dirpages,
 * so that each page can be used as a single logical lu_dirpage.
 *
 * A lu_dirpage is laid out as follows, where s = ldp_hash_start,
 * e = ldp_hash_end, f = ldp_flags, and each "ent" is a struct lu_dirent.
 * It has size up to LU_PAGE_SIZE. The ldp_hash_end value is used to
 * request the next lu_dirpage in a directory listing that spans multiple
 * pages (two in this example):
 *   ________
 *  |        |
 * .|--------v-------   -----.
 * |s|e|f|p|ent|ent| ... |ent|
 ...
 * |        next CFS_PAGE       |
 *
 * This structure is adjusted as follows:
 *
 * - Replace e0 with e' so when we request the next lu_dirpage we will get
 *   the page labeled 'next CFS_PAGE'.
 *
...
static void lmv_adjust_dirpages(struct page **pages, int nrdpgs, int nlupgs)
{
	int i;
...
#else
#define lmv_adjust_dirpages(p, d, l) do {} while (0)
			...
			struct page **pages, struct ptlrpc_request **request)
{
	...
	int			rc;
	int			nrdpgs; /* pages read in CFS_PAGE_SIZE */
	...
	lmv_adjust_dirpages(pages, nrdpgs, nlupgs);
