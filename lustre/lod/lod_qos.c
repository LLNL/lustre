                        ...
                        struct lu_attr *attr, int flags, struct thandle *th)
{
        ...
        if (lo->mbo_pool)
                pool = lod_find_pool(m, lo->mbo_pool);
...
__u16 lod_get_stripecnt(struct lod_device *lod, __u32 magic,
                        __u16 stripe_count)
{
                                    ...
                                    const struct lu_buf *buf)
{
        ...
        mo->mbo_stripenr = le16_to_cpu(v1->lmm_stripe_count);
        mo->mbo_layout_gen = le16_to_cpu(v1->lmm_layout_gen);
                        ...
                        struct thandle *th)
{
        ...
        if (likely(lo->mbo_stripe == NULL)) {
                ...
                lo->mbo_stripenr = lod_get_stripecnt(d, LOV_MAGIC,
                                                     lo->mbo_stripenr);
