...
/* This function is _only_ for test b=21846 */
int lustre_hash_get_cur_bits(lustre_hash_t *lh)
{
        ...
        lh_read_lock(lh);
        cur_bits = lh->lh_cur_bits;
        lh_read_unlock(lh);

        return cur_bits;
}
EXPORT_SYMBOL(lustre_hash_get_cur_bits);
...
int lustre_hash_set_cur_bits(lustre_hash_t *lh, int cur_bits)
{
        ...
        lh_write_lock(lh);
        if (lh->lh_cur_bits != cur_bits) {
                lh->lh_min_bits = MIN(lh->lh_min_bits, cur_bits);
                lh->lh_max_bits = MAX(lh->lh_max_bits, cur_bits);
                lh_write_unlock(lh);
                lustre_hash_rehash(lh, cur_bits);
                return 0;
        }
        lh_write_unlock(lh);
...
EXPORT_SYMBOL(lustre_hash_set_cur_bits);
