                                               ...
                                               struct dt_device *dev)
{
        ...
        lgi->lgi_fid.f_oid = 1;
        lgi->lgi_fid.f_ver = 0;
        rc = local_oid_storage_init(env, dev, &lgi->lgi_fid, &lsb->lsb_los);
        if (rc) {
...
static int llog_osd_generate_fid(const struct lu_env *env,
                                 ...
                                 struct lu_fid *fid)
{
        return local_object_fid_generate(env, lsb->lsb_los, fid);
}
                                   ...
                                   struct thandle *th)
{
        ...
        lsb = res->private_data;
        LASSERT(lsb);

        rc = llog_osd_declare_new_object(env, lsb->lsb_los, o, th);
        if (rc)
                RETURN(rc);
                           ...
                           struct thandle *th)
{
        ...
        if (!dt_object_exists(o))
                rc = llog_osd_create_new_object(env, lsb->lsb_los, o, th);
        else
                rc = -EEXIST;
        if (res->lgh_name)
                dt_ref_add(env, o, th);
