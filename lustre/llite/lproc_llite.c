                               ...
                               int count, int *eof, void *data)
{
	...
	*eof = 1;
	...
	return snprintf(page, count,
			"llite_lru [ users: %d, max_cached_mb: %d, "
			"used_mb: %d, unused_mb: %d, reclaim_count: %u ].\n",
			cfs_atomic_read(&lru->ccl_refc),
			max_cached_mb, max_cached_mb - unused_mb, unused_mb,
                               ...
                               unsigned long count, void *data)
{
	...
	if (pages_number < 0 || pages_number > cfs_num_physpages) {
		char *fsname = ll_get_fsname(sbi);
		CERROR("%s: can't set max cache more than %lu MB\n",
			fsname, cfs_num_physpages >> (20 - CFS_PAGE_SHIFT));
