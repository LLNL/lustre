                        ...
                        struct llog_changelog_rec *rec, struct thandle *th)
{
        ...
        rc = llog_add(env, ctxt->loc_handle, &rec->cr_hdr,
                      NULL, NULL, th);
        llog_ctxt_put(ctxt);
...
{
        int rc = 0;
        int is_dir = S_ISDIR(la->la_mode);
        ENTRY;

        LASSERT(mdd_write_locked(env, obj) != 0);

        if (la->la_nlink == 0 || is_dir) {
                ...
                /* get the fresh nlink */
                mdd_la_get(env, obj, la, mdd_object_capa(env, obj));
        }
                      ...
                      struct md_attr *ma)
{
        ...
        rc = mdd_finish_unlink(env, mdd_cobj, cattr, handle);
#ifdef HAVE_QUOTA_SUPPORT
                      ...
                      struct md_attr *ma)
{
        ...
        if (tobj && mdd_object_exists(mdd_tobj)) {
                ...
                if (rc)
                        GOTO(fixup_tpobj, rc);
                rc = mdd_finish_unlink(env, mdd_tobj, tg_attr, handle);
                mdd_write_unlock(env, mdd_tobj);
                if (rc)
                        GOTO(fixup_tpobj, rc);
#ifdef HAVE_QUOTA_SUPPORT
