...
static int __mds_lov_synchronize(void *data)
{
        ...
        if (rc != 0) {
                LCONSOLE_ERROR("%s: Failed to clear orphan objects on OST: %d\n",
                               obd_uuid2str(uuid), rc);
                GOTO(out, rc);
        }
...
out:
        if (rc) {
                ...
                LCONSOLE_ERROR("%s sync failed %d, deactivating\n",
                               obd_uuid2str(uuid), rc);
               ...
               enum obd_notify_event ev, void *data)
{
        ...
        if (obd->obd_recovering) {
                LCONSOLE_WARN("%s: in recovery, not resetting orphans on "
                              "%s\n", obd->obd_name, obd_uuid2str(
                              &watched->u.cli.cl_target_uuid));
