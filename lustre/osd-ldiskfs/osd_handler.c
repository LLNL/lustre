                                ...
                                struct lustre_capa *capa)
{
        ...
        if (rc >= 0) {
                ...
                iam_reccpy(&it->ii_path.ip_leaf, iam_rec);
                if (S_ISDIR(obj->oo_inode->i_mode))
                            ...
                            struct htree_lock *hlock, struct thandle *th)
{
	...
	/* local file compat names are created by OSD but may be created by
	 * MDD, e.g. for ROOT. No need to insert name again */
	if (unlikely(fid_seq((struct lu_fid *)fid) == FID_SEQ_LOCAL_FILE))
		RETURN(0);
