                                 ...
                                 struct osd_it_quota *it)
{
        ...
        struct lustre_disk_dqblk_v2 *ddquot =
                (struct lustre_disk_dqblk_v2 *)GETENTRIES(buf);
        int dqblk_sz = sizeof(struct lustre_disk_dqblk_v2);
        ENTRY;

        if (!buf)
                RETURN(-ENOMEM);
        if ((ret = quota_read_blk(env, obj, type, blk, buf)) < 0) {
        ...
        if (dqid) {
                for (i = 0; i < LUSTRE_DQSTRINBLK &&
                     le32_to_cpu(ddquot[i].dqb_id) != dqid;
                     i++) ;
        } else { /* ID 0 as a bit more complicated searching... */
                for (i = 0; i < LUSTRE_DQSTRINBLK; i++)
                        if (!le32_to_cpu(ddquot[i].dqb_id)
                            && memcmp((char *)&emptydquot,
                         ...
                         struct osd_it_quota *it)
{
        ...
        if (!buf)
                RETURN(-ENOMEM);
        if ((ret = quota_read_blk(env, obj, 0, blk, buf)) < 0) {
                CERROR("Can't read quota tree block %u.\n", blk);
                GOTO(out_buf, ret);
        }
        ret = 0;
                       ...
                       struct osd_it_quota *it)
{
        ...
        int i, dqblk_sz = sizeof(struct lustre_disk_dqblk_v2);
        struct lustre_disk_dqblk_v2 *ddquot;
        ENTRY;
