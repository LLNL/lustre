                                ...
                                struct mdt_object *o, int ma_need)
{
        ...
        if (mdt_body_has_lov(la, reqbody)) {
                ...
                if (unlikely(rc <= 0)) {
                        ...
                        if (rc != reqbody->eadatasize - 1)
                                CERROR("Read shorter link %d than expected %d\n",
...
static int mdt_stack_init(const struct lu_env *env, struct mdt_device *mdd,
                          struct lustre_cfg *cfg)
{
        ...
        strncpy(mdd->mdt_nextdev, name, 128);
...
static int mdt_connect_to_next(const struct lu_env *env, struct mdt_device *m)
{
        ...
        obd = class_name2obd(m->mdt_nextdev);
        if (obd == NULL) {
        ...
        rc = obd_connect(NULL, &m->mdt_child_exp, obd, &obd->obd_uuid, data,
                         NULL);
        if (rc) {
        ...
        site = m->mdt_child_exp->exp_obd->obd_lu_dev->ld_site;
        LASSERT(site);
        LASSERT(m->mdt_md_dev.md_lu_dev.ld_site == NULL);
        m->mdt_md_dev.md_lu_dev.ld_site = site;
        site->ls_top_dev = &m->mdt_md_dev.md_lu_dev;
