...
# set hash_cur_bits
set_quota_hash_cur_bits() {
        local bits=$1
        local ostcount=`lctl get_param -n lov.$LOVNAME.numobd`

        # set quota_hash_cur_bits on all obdfilters
        for num in `seq $OSTCOUNT`; do
	    do_facet ost$num "lctl set_param lquota.${FSNAME}-OST*.quota_hash_cur_bits=$bits"
        done
        # set quota_hash_cur_bits on mds
        do_facet mds  "lctl set_param lquota.${FSNAME}-MDT*.quota_hash_cur_bits=$bits"
}
