...
int cl_init_ea_size(struct obd_export *md_exp, struct obd_export *dt_exp)
{
        ...
        int rc, easize, def_easize, cookiesize, def_cookiesize;
	...
	def_stripes = min(desc.ld_default_stripe_count, (__u32)LOV_MAX_STRIPE_COUNT);
	lsm.lsm_stripe_count = def_stripes;
	def_easize = obd_size_diskmd(dt_exp, &lsm);

	cookiesize = stripes * sizeof(struct llog_cookie);
	def_cookiesize = def_stripes * sizeof(struct llog_cookie);

	CDEBUG(D_HA,
	       "updating def/max_easize: %d/%d def/max_cookiesize: %d/%d\n",
	       def_easize, easize, def_cookiesize, cookiesize);
