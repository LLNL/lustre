...
static inline int
lnet_portal_is_rdma(lnet_portal_t *ptl)
{
        return !!(ptl->ptl_options & LNET_PTL_RDMA); 
}

static inline int
lnet_portal_is_request(lnet_portal_t *ptl)
{
        return !!(ptl->ptl_options & LNET_PTL_REQUEST);
}

static inline void
lnet_portal_set(lnet_portal_t *ptl, int opt)
{
        ptl->ptl_options |= opt;
}

static inline void
lnet_portal_unset(lnet_portal_t *ptl, int opt)
{
        ptl->ptl_options &= ~opt;
}
...
lnet_portal_me_head(int index, lnet_process_id_t id, __u64 mbits)
{
        ...
        if (lnet_portal_is_request(ptl)) {
                return &ptl->ptl_mlist;
        } else if (lnet_portal_is_rdma(ptl)) {
