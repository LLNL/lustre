...
/* assume all nodes can understand LST_PROTO_VERSION_1 protocol */
static unsigned            session_version = LST_PROTO_VERSION_1;
#else
static unsigned            session_version = LST_PROTO_VERSION_0;
#endif
...
lst_new_session_ioctl(char *name, int timeout, int force, lst_sid_t *sid)
{
        ...
        args.lstio_ses_version = session_version;
...
jt_lst_new_session(int argc,  char **argv)
{
        ...
        fprintf(stdout, "SESSION: %s VERSION: %u TIMEOUT: %d FORCE: %s\n",
                name, session_version, timeout, force ? "Yes": "No");
        return 0;
}

int
lst_session_info_ioctl(char *name, int len, int *key, unsigned *verp,
...
jt_lst_show_session(int argc, char **argv)
{
        ...
        fprintf(stdout, "%s ID: "LPU64"@%s, KEY: %d VERSION: %u NODES: %d\n",
                name, sid.ses_stamp, libcfs_nid2str(sid.ses_nid),
                key, version, ndinfo.nle_nnode);
...
lst_add_nodes_ioctl (char *name, int count, lnet_process_id_t *ids,
                     unsigned *verp, cfs_list_t *resultp)
{
...
jt_lst_add_group(int argc, char **argv)
{
        ...
        char              *name;
        unsigned           version = session_version;
        int                count;
        int                rc;
        int                i;
        ...
        for (i = 2; i < argc; i++) {
                ...
                rc = lst_add_nodes_ioctl(name, count, ids, &version, &head);
                ...
                if (version != session_version) {
                        fprintf(stdout,
                                ...
                                "compatible mode because session version "
                                "on some test nodes are V%u, which is"
                                "lower than expected version V%u, you "
                                "may get issue with some commands\n",
        ...
        if (rc == -1) {
                ...
                if (trans_stat.trs_fwk_errno == EPROTO) {
                        fprintf(stderr,
                                "session version on remote node is too old, "
                                "or mixed session versions on some test nodes, "
                                "please set LST_VERSION to lower version "
                                "number and recreate the session\n");
                }
...
lst_initialize(void)
{
        ...
        version = getenv("LST_VERSION");
        if (version != NULL)
                session_version = atoi(version);

        if (session_version > LST_PROTO_VERSION) {
                fprintf(stderr,
                        "Unsupported session version number %u, "
                        "please use %u or lower version\n",
                        session_version, LST_PROTO_VERSION);
                return -1;
        }
