...
brw_client_init (sfw_test_instance_t *tsi)
{
        ...
        if (sn->sn_version == LST_PROTO_VERSION_0) {
                 ...
                 * but we have to keep it for version compatibility */
                len   = npg * CFS_PAGE_SIZE;

        } else {
                test_bulk_req_v1_t  *breq = &tsi->tsi_u.bulk_v1;

                /* I should never get this step if it's unknown version
                 * because make_session will reject unknown version */
        ...
        cfs_list_for_each_entry_typed (tsu, &tsi->tsi_units,
                                       sfw_test_unit_t, tsu_list) {
                bulk = srpc_alloc_bulk(npg, len, opc == LST_BRW_READ);
                if (bulk == NULL) {
                        brw_client_fini(tsi);
                        return -ENOMEM;
                }
                     ...
                     lnet_process_id_t dest, srpc_client_rpc_t **rpcpp)
{
        ...
        rc = sfw_create_test_rpc(tsu, dest, sn->sn_version, npg, len, &rpc);
        if (rc != 0)
                return rc;
...
brw_server_handle (srpc_server_rpc_t *rpc)
{
        ...
        if (reqstmsg->msg_session_ver == LST_PROTO_VERSION_0) {
        ...
        } else if (reqstmsg->msg_session_ver == LST_PROTO_VERSION) {
        ...
        rc = sfw_alloc_pages(rpc, npg, reqst->brw_len,
