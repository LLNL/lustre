...
srpc_bulk_t *
srpc_alloc_bulk(unsigned bulk_npg, unsigned bulk_len, int sink)
{
        srpc_bulk_t  *bk;
        cfs_page_t  **pages;
        int           i;
        ...
        LIBCFS_ALLOC(bk, offsetof(srpc_bulk_t, bk_iovs[bulk_npg]));
        if (bk == NULL) {
        ...
        bk->bk_niov   = bulk_npg;
#ifndef __KERNEL__
        LIBCFS_ALLOC(pages, sizeof(cfs_page_t *) * bulk_npg);
        if (pages == NULL) {
