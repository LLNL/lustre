                       ...
                       const char *format, ...)
{
        ...
        rc = libcfs_debug_vmsg2(msgdata, format, args, NULL, NULL);
        va_end(args);
...
void
libcfs_assertion_failed(const char *expr, libcfs_debug_msg_data *msgdata)
{
        libcfs_debug_msg(msgdata, D_EMERG, "ASSERTION(%s) failed\n", expr);
        abort();
}
