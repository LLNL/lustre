...
int
cfs_hash_for_each_nolock(cfs_hash_t *hs, cfs_hash_for_each_cb_t func,
			 void *data, int start)
{
        ENTRY;
...
int cfs_hash_debug_str(cfs_hash_t *hs, struct seq_file *m)
{
	...
	c += seq_printf(m, "%5d %5d %5d ", 1 << hs->hs_cur_bits,
			1 << hs->hs_min_bits, 1 << hs->hs_max_bits);
	c += seq_printf(m, "%d.%03d ",
	...
	c += seq_printf(m, " 0x%02x %6d ", hs->hs_flags,
			hs->hs_rehash_count);
	...
	c += seq_printf(m, "%7d %7d %7d ", total, maxdep, maxdepb);
	for (i = 0; i < 8; i++)
